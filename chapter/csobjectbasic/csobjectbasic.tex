...
类(class) 是面向对象程序设计的基础与核心，
类也是 \cs 语言最基础最重要的类型，是一种将数据（字段或属性）与对数据的操作（方法、函数等）组合在一起的数据结构。


\section{类(class)}

\subsection{类与实例对象(instance object)}

类(class)是创建对象的蓝图或模板，对象(object)是根据类创建的具体实例(instance)。
我们以点(Point)与具体的某个点(如11号教学楼旁的那个名叫 GP12 的点)为例，
点就是类，它是一个抽象的概念，定义了点的属性与行为，而具体点 GP12 就是点的实例对象。




\subsection{类的实例与静态类、静态属性(static)}

不是所有的类都有实例对象。没有实例或不需要实例的类定义为静态类(static class)。
一个被定义为静态的类不能生成类的实例。如 \cs 中的数学类 System.Math,
在这个类中定义个各种常数与常用数学算法，但没有任何实例数据和方法，
因此将其定义为静态类(static class)。

一个实例类中也可以有静态成员(static member)，静态成员属于类本身，而不是类的实例，
\cs 中在成员前加关键字 static 将其定义为静态成员。

类的静态成员调用方法为： 类名.静态成员名 ，
而类的实例成员调用方法为： 实例对象名.实例成员名 。

在类的静态方法中不能直接调用非静态成员，需通过实例对象进行调用；
但在实例方法中可以直接调用静态成员。

在此声明，若非特别语境下，以后本书中所指的类均为实例类。


\subsection{类的定义}

类的关键字为 class ，定义一个类的方式如下：
...
先指定类的修饰符， class 关键字后是类名称（类名首字母一般大写），接着是要继承的基类
与需实现的接口。
...
下面的语句是生成 Point 实例对象，可以合理使用 \cs 的推断功能，
在较新的 \cs 语法中， 在知道类型的情况下，可以简化 new 的写法：
...
当程序执行到这段代码后边的 ``\}'' 时，将自动释放变量 p1、p2、p3，
p1、p2与p3所指向的内存，将由 .NET 系统自动收回。

\subsection{类的字段（Filed）}
类的字段用于存储类的数据，是与类或类的实例相关的变量。
...
一个类可以访问自己的属性，可以通过 this 关键字进行引用访问，在不引起混淆的情况下，可以省略 this。
this 关键字指类的实例本身。
...
%% { } 用于限定  \heiti 的作用范围, 或用 \textheiti{ 这里是黑体 }
{ \heiti 构造函数是函数名称与类名完全相同，且没有返回类型(即使返回类型为void也不行)的函数，
...
\cs 中类的成员如表 \ref{tab:classcontentitems} 所示：
...
\input{chapter/csobjectbasic/surveyfunctions}