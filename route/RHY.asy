...
pair HZ = rotate(180-_alpha, JD)*ZH; //将ZH点绕JD点旋转180°-55°得到HZ点
...
draw(Label("$y$",EndPoint), ZH--OE, E, black, Arrow); 


pair pm = (_p, _m);
//dot("$pm$",pm, E);
draw(pm--O, dashed);
pair mp = rotate(-_alpha, O)* pm; //将(p,m)点绕O点旋转55°得到(m,p)点
//dot("$mp$",mp, E);
draw(mp--O, dashed);

draw(arc(O,pm, mp, CW), magenta+linewidth(0.3) + dashed);

pair HY =(_l0^2 / 6 / _R - _l0^4 / 336 / _R^3, _l0 - _l0^3 / 40 / _R^2);
draw(HY--O, dashed);

real beta = _alpha - _beta0*2;
pair YH = rotate(-beta,O)* HY; //将HY点绕O点旋转55°/2得到YH点
draw(YH--O, dashed);

//圆心，起点，方向点
draw(arc(O, HY, YH, CW), magenta+linewidth(2)); 
...
pair ROYY = HZ + 1*dir(ROY--HZ); 
draw(Label("$y'$",EndPoint), ROY--ROYY, red, Arrow); 

draw(Label("$\alpha$", red, align=(0,0), filltype=Fill(white)),
 arc(ON,JD,HZ,0.6),red,Arrows);

dot("$ZH$",ZH, W, darkblue+0.3mm, UnFill);
dot("$JD$",JD, SW, darkblue+0.3mm, UnFill);
dot("$O$",O, SE, darkblue+0.3mm, UnFill);
dot("$QZ$",QZ, E, darkblue+0.3mm, UnFill);
dot("$HZ$",HZ, E, darkblue+0.3mm, UnFill);
dot("$HY$",HY, NW, darkblue+0.3mm, UnFill);
dot("$YH$",YH, NW, darkblue+0.3mm, UnFill);
dot("$O'$",OT, N, red+0.3mm, UnFill);
